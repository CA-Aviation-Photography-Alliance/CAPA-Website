...
const BASE_URL = 'http://localhost:3001/api/auth-events';

// Mock JWT token for testing (replace with real token from Auth0)
const MOCK_TOKEN = 'eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6IktFWV9JRCJ9..SIGNATURE';
...
async function testAuth0API() {
  ...
  console.log('   Replace MOCK_TOKEN with a real token from your Auth0 application\n');
  ...
  try {
    ...
    console.log('Test 3: Try to access /my endpoint without auth (should fail)');
    await authApiRequest(`${BASE_URL}/my`);

    // Test 4: Create event with mock token (will fail with real Auth0 validation)
    console.log('Test 4: Create event with mock token (will fail with real validation)');
    const createResponse = await authApiRequest(BASE_URL, {
      method: 'POST',
      body: JSON.stringify({
        startdate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
        enddate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000 + 2 * 60 * 60 * 1000).toISOString(),
    ...
    if (eventId) {
      ...
      await authApiRequest(`${BASE_URL}/${eventId}`, {
        method: 'PATCH',
        body: JSON.stringify({
    ...
    if (eventId) {
      ...
      const wrongUserToken = MOCK_TOKEN.replace('auth0|1234567890', 'auth0|0987654321');
      await authApiRequest(`${BASE_URL}/${eventId}`, {
        method: 'PATCH',
        body: JSON.stringify({
...
function checkEnvironment() {
  ...
  if (!allSet) {
    ...
    console.log('   Copy .env.auth0-example to .env and fill in your Auth0 settings');
    console.log('   Required variables: AUTH0_DOMAIN, AUTH0_AUDIENCE, AUTH0_CLIENT_ID\n');
  } else {
...
async function main() {
  ...
  console.log('- POST   /api/auth-events           - Create event (auth required)');
  console.log('- GET    /api/auth-events/my        - Get my events (auth required)');
  console.log('- PUT    /api/auth-events/:id       - Update event (ownership required)');
  console.log('- PATCH  /api/auth-events/:id       - Partial update (ownership required)');
  console.log('- DELETE /api/auth-events/:id       - Delete event (ownership required)');
  console.log('');
