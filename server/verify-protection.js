...
async function verifyProtection() {
  ...
  try {
    ...
    console.log(`   Without creator: ${updateNoCreator.status} ${updateNoCreator.ok ? '❌ FAILED' : '✅ BLOCKED'}`);

    // Try update with wrong creator (should fail)
    const updateWrongCreator = await fetch(`${BASE_URL}/${eventId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: 'Wrong Creator Update', creator: 'Hacker' })
    });
    console.log(`   Wrong creator: ${updateWrongCreator.status} ${updateWrongCreator.ok ? '❌ FAILED' : '✅ BLOCKED'}`);
    ...
    console.log(`   Correct creator: ${updateCorrect.status} ${updateCorrect.ok ? '✅ ALLOWED' : '❌ FAILED'}\n`);
    ...
    console.log(`   Without creator: ${deleteNoCreator.status} ${deleteNoCreator.ok ? '❌ FAILED' : '✅ BLOCKED'}`);

    // Try delete with wrong creator (should fail)
    const deleteWrongCreator = await fetch(`${BASE_URL}/${eventId}?creator=Hacker`, {
      method: 'DELETE'
    });
    console.log(`   Wrong creator: ${deleteWrongCreator.status} ${deleteWrongCreator.ok ? '❌ FAILED' : '✅ BLOCKED'}`);

    // Try delete with correct creator (should succeed)
    const deleteCorrect = await fetch(`${BASE_URL}/${eventId}?creator=TestUser`, {
      method: 'DELETE'
    });
    console.log(`   Correct creator: ${deleteCorrect.status} ${deleteCorrect.ok ? '✅ ALLOWED' : '❌ FAILED'}\n`);

    // Step 4: Summary
    console.log('📋 Protection Status:');
    console.log('====================');

    if (updateNoCreator.status === 400 && updateWrongCreator.status === 403 && updateCorrect.ok) {
      console.log('✅ UPDATE protection: WORKING');
    } else {
      console.log('❌ UPDATE protection: NOT WORKING');
    }

    if (deleteNoCreator.status === 400 && deleteWrongCreator.status === 403 && deleteCorrect.ok) {
